			...
			<div class="col-md-3">
               ...
               @if($client->image != "")
					...
					@if(Wingu::business()->plan != 1)
						<a href="{!! route('crm.customer.mail',$customerID) !!}" class="btn btn-primary btn-xs waves-effect mb-2 waves-light"><i class="fal fa-paper-plane"></i> Send Email</a>
					@endif
			...
			<div class="col-md-9">
				<div class="row mb-3">
                  ...
                  <div class="float-right">
							<a href="{{ route('crm.customers.edit', $client->cid) }}" class="btn btn-white"><i class="fas fa-edit"></i> Edit</a>
							@if(Wingu::business()->plan != 1)
   						...
   						<div class="btn-group">
								...
								<ul class="dropdown-menu">
									@if(Wingu::business()->plan != 1)
										<li><a href="{!! route('crm.customer.sms',$customerID) !!}"><i class="fal fa-sms"></i> SMS</a></li>
										<li><a href="{!! route('crm.customer.mail',$customerID) !!}"><i class="fal fa-paper-plane"></i> Mail</a></li>
										<li class="divider"></li>
									@endif
									<li><a href="{!! route('crm.customer.notes',$customerID) !!}"><i class="fal fa-sticky-note"></i> Notes</a></li>
									<li><a href="{!! route('crm.customer.events',$customerID) !!}"><i class="fal fa-calendar-alt"></i> Meeting</a></li>
									<li><a href="{!! route('crm.customer.calllog',$customerID) !!}"><i class="fal fa-phone-office"></i> Call log</a></li>
									<li class="divider"></li>									
									<li><a href="{!! route('crm.customers.delete', $client->cid) !!}"><i class="fas fa-trash"></i> Delete</a></li>
								</ul>
							</div>
							@if(Wingu::business()->plan != 1)
								<a href="{!! route('crm.customer.documents',$customerID) !!}" class="btn btn-white"><i class="fal fa-folder"></i> Documents</a>
							@endif
                  </div>
					</div>
				</div>
				...
				<!-- main client page -->
				@if(Request::is('crm/customer/'.$customerID.'/show'))
					@include('app.crm.customers.show')					
				@endif

				<!-- comments -->
				@if(Request::is('crm/customer/'.$customerID.'/comments'))
					@include('app.crm.customers.comments')
				@endif

				<!-- invoice -->
				@if(Request::is('crm/customer/'.$customerID.'/invoices'))
					@include('app.crm.customers.invoices')
				@endif

            <!-- estimates -->
				@if(Request::is('crm/customer/'.$customerID.'/quotes'))
					@include('app.crm.customers.quotes')
				@endif

            <!-- creditnote -->
				@if(Request::is('crm/customer/'.$customerID.'/creditnotes'))
					@include('app.crm.customers.creditnotes')
				@endif

				<!-- projects -->
				@if(Request::is('crm/customer/'.$customerID.'/projects'))
					@include('app.crm.customers.projects')
				@endif

				<!-- statement -->
				@if(Request::is('crm/customer/'.$customerID.'/statement'))
					@include('app.crm.customers.statement')
				@endif

				<!-- statement mail -->
				@if(Request::is('crm/customer/'.$customerID.'/statement/mail'))
					@include('app.crm.customers.statementMail')
				@endif

				<!-- subscription -->
				@if(Request::is('crm/customer/'.$customerID.'/subscriptions'))
					@include('app.crm.customers.subscriptions')
				@endif

            <!-- contacts -->
				@if(Request::is('crm/customer/'.$customerID.'/contacts'))
					@include('app.crm.customers.contacts')
				@endif

				<!-- mails -->
				@if(Request::is('crm/customer/'.$customerID.'/mail'))
					@include('app.crm.customers.mail.index')
				@endif

				<!-- send -->
				@if(Request::is('crm/customer/'.$customerID.'/send'))
					@include('app.crm.customers.mail.mail')
				@endif

				<!-- documents -->
				@if(Request::is('crm/customer/'.$customerID.'/documents'))
					@include('app.crm.customers.documents')
				@endif

				<!-- sms -->
				@if(Request::is('crm/customer/'.$customerID.'/sms'))
					@include('app.crm.customers.sms')
				@endif

				<!-- notes -->
				@if(Request::is('crm/customer/'.$customerID.'/notes'))
					@include('app.crm.customers.notes')
				@endif

				<!-- events -->
				@if(Request::is('crm/customer/'.$customerID.'/events'))
					@include('app.crm.customers.events')
				@endif

				<!-- events -->
				@if(Request::is('crm/customer/'.$customerID.'/calllogs'))
